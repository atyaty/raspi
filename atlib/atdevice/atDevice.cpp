    ...
    if(n>ParentSzereg){
        ...
        for(int i=0;i<WeSize();i++){
            if(aWe.at(i)->devExternal()!=NULL){
                atDevice *dP=aWe.at(i)->devExternal()->GetBrother(ParentDev);//patrz opis GetBrother
...
bool atDevice::connect(atDevice *d, unsigned nWy, unsigned nWe) {
    ...
    atDevice *dP=d->GetBrother(ParentDev);
...
//---------------------------------------------------------
// funkcja szuka brata :)
// czyli devica który ma tego samego rodzica: d
// jako parametr przekazany jest wskaźnik dokumentu nadrzędnego: d
// jeśli naszym rodzicem jest d to znaczy, że my sami jesteśmy szukanym bratem
// jeśli nie to sprawdzamy czy nasz rodzic nie jest bratem rekurencyjnie
// w ten sposób jeśli któryś z naszych pra-rodziców będzie miał d za rodzica, to znaczy że on jest bratem
// innymi słowy: d jest punktem odniesienia a bratem jest ten z naszej rodziny kto jest bezpośrednio pod punktem d
// ta funkcja jest uzywana do szeregowania Partsów
atDevice *atDevice::GetBrother( atDevice *d) {
  if(ParentDev==d) return this; //kolejność ifów jest ważna, daje nam to że dla d==NULL mamy brata a nie NULL
  if(ParentDev==NULL) return NULL;
  return ParentDev->GetBrother(d);
}
