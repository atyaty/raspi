...
bool programSD::goPomiar(){
    ...
    if(stat_pomiar==SD_POMIAR_START)        pomiarStart();
    else if(stat_pomiar==SD_POMIAR_ZWLOKA)  pomiarZwloka();
    else if(stat_pomiar==SD_POMIAR_GETDATA) pomiarGetData();
    else if(stat_pomiar==SD_POMIAR_WYNIK)   pomiarWynik();
    else if(stat_pomiar==SD_POMIAR_NEXT)    pomiarNext();
    else if(stat_pomiar==SD_POMIAR_SETWAVE) pomiarSetWave();
    else if(stat_pomiar==SD_POMIAR_STOP)    {Rap("stop Pomiar",AT_LOG_CLASSMAIN); stat=SD_STATUS_STOP; }
...
bool programSD::pomiarStart(){
    Rap("pomiarStart",AT_LOG_CLASSMAIN);
    ...
    if( X3Motor->go() ){
        ...
        if(dnStop){
            Freq=Freq_min;
            Ampl=Ampl_min;
            stat_pomiar=SD_POMIAR_SETWAVE;
        }
        else{
            stat_pomiar=SD_POMIAR_STOP;
            Error=ATERROR_SD_NOT_DOWN;
        }
    }
    else{
        stat_pomiar=SD_POMIAR_STOP;
        Error=ATERROR_SD_X3COMMUNICATION;
    }
    return true;
}
//-------------------------------------------------
bool programSD::pomiarSetWave(){
    Rap("pomiarSetWave Freq="+to_string(Freq)+" Ampl="+to_string(Ampl) , AT_LOG_DEBUG);
    atDataBuffer* dbw=X3Wave->getDbw();
    dbw->Clear();
    dbw->buforSetNag(AT_MODBUS_SET,X3Wave->getAddrDev(),0,X3WaveDataLen);
    dbw->buforAddReg(Ampl);
    dbw->buforAddReg(Freq);
    dbw->buforAddCRC();
    X3Wave->setSendIsHot();
    if( X3Wave->go() ){
        stat_pomiar=SD_POMIAR_ZWLOKA;
        stoper.start_ms(SD_SAMPLE_TIME);
    }
    else{
        stat_pomiar=SD_POMIAR_STOP;
        Error=ATERROR_SD_X3_SETWAVE;
    }
    return true;
}
//-------------------------------------------------
bool programSD::pomiarNext(){
    uint32_t F=Freq,A=Ampl;
    F+=Freq_d;
    if(F>Freq_max || Freq_d==0){F=Freq_min; A+=Ampl_d;}
    if(A>Ampl_max || Ampl_d==0){
        stat_pomiar=SD_POMIAR_FINISHED;
        Rap("..Finished." , AT_LOG_DEBUG);
        return true;
    }  //to jest pułapka. pomiar wisi na statusie "finished" dopóki nie przyjdzie polecenie kończące pomiar
    Freq=F; Ampl=A;
    Rap("pomiarNextPoint: Freq="+to_string(Freq)+" Ampl="+to_string(Ampl)  , AT_LOG_DEBUG);
    stat_pomiar=SD_POMIAR_SETWAVE;
    return true;
}
//-------------------------------------------------
bool programSD::pomiarZwloka(){
    ads->scan();
    if( ads->getChannel()!=0 || ads->getADC_N(0)==0){ return false; }
    ads->next();

    for(int i=0;i<ads->getLen();i++){     //
        int32_t x=ads->getADC_buforSr(i); // atrapa - dla wyrównania  z PomiarGetData -wywalić pętlę
    }                                     //

    if( !stoper.waiting() ){
        Rap("pomiarZwloka ..OK" , AT_LOG_DEBUG);
        nWrite=0;
        nSend=0;
        pomiar_time=0;
        stat_pomiar=SD_POMIAR_GETDATA;
        stoper.start_ms(SD_SAMPLE_TIME);
    }
    return true;
}
//----------------------------------------------------------------------------------
// UWAGA!
// w SD wszystkie kanały czytają ten sam sygnał!
// skanujemy dopóki nie zapełnią się bufory wszystkich kanałów
// istotna jest dla nas kolejność pobieranych próbek
// kanały powinny być odczytane w takiej kolejności w jakiej zostały zapisane
// ponieważ odczyt jest od zerowego kanału, to procedura odczytu powinna być uruchomiona w momencie
// zapełnienia ostatniego kanału, czyli gdy getChannel wskazuje 0.
//---------------------------------------------------------------------
bool programSD::pomiarGetData() {
    ...
    //string debug_s="pomiarGetData ("+to_string(nWrite)+") ";

    for(int i=0;i<ads->getLen();i++){
        input[nWrite]=ads->getADC_buforSr(i);
        //debug_s+=to_string(input[nWrite])+"; ";
        //if(i==7)Rap(debug_s);
        nWrite++;

        if( nWrite>=SD_BUFOR_LEN ){
            pomiar_time=stoper.time_ms();
            stat_pomiar=SD_POMIAR_WYNIK;
            Rap("pomiarGetData -finished -time: "+to_string(pomiar_time)+"   nSend: "+to_string(nSend)+"   nWrite: "+to_string(nWrite) , AT_LOG_DEBUG);
        }
    }
    return true;
}
...
bool programSD::pomiarWynik(){
    ...
    //Rap("pomiarWynik pomiar_time: "+to_string(pomiar_time));
    if(nSend>=SD_BUFOR_LEN){
        stat_pomiar=SD_POMIAR_NEXT;
        Rap("pomiarWynik - goNext,  time: "+to_string(pomiar_time) , AT_LOG_DEBUG);
    }
    else{
       // Rap("pomiarWynik czekanie na SS, nSend: "+to_string(nSend)+" nWrite:"+to_string(nWrite));
    }
    return true;
}
...
bool programSD::upStart(){
    //Rap("upStart,stat: "+to_string(stat)+", stat_up: "+to_string(stat_up)+" \n");
    //atDataBuffer* dbr=X3Motor->getDbr();
    atDataBuffer* dbw=X3Motor->getDbw();
    dbw->Clear();
    ...
    if( X3Motor->go() ){
        stat_up=SD_UP_WAIT;
        //Rap("upStart- wait,stat: "+to_string(stat)+", stat_up: "+to_string(stat_up)+" \n");
    }
    else{
        stat_up=SD_UP_STOP;
